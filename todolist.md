...
- [x] 提供上游安装 Skill、固定 commit 的 submodule、project.yaml 和目标运行时 Skill；不自动 commit/push，不切换用户分支。
- [x] 安装器支持 dry-run、重复执行、保留 AGENTS 原文、冲突拒绝与构建失败重试。
- [x] 补齐 check JSON Schema；覆盖 staged/unstaged/untracked、删除与重命名。
...
设计已确认，第一项 domain 模型与校验已实现，其余闭环工作仍待完成。实施计划：[规则内核 v2](docs/superpowers/plans/2026-09-08-rule-kernel-v2.md)、[安装进度与超时](docs/superpowers/plans/2026-09-08-integration-progress-timeouts.md)。

- [x] 添加不可变层级／profile 模型、显式通用选择和范围校验；domain 33 项测试及独立审查已通过（2026-09-08）。
...
- [x] 规范器 `ftckb check`：对 diff 做确定性执法（path 看触及路径，regex 看新增行）；当前 5 条生效规则带硬检查，含跨赛季 `global.test-utility-layout` 对错误 TeamCode 路径/JUnit 新增的 exit 1 拦截；完整 Agent instruction 仍负责 tests/utils 的语义分类。无 checks/无 trigger 为无条件 soft，Limelight validity/freshness 与 V0.7.0 `global.vendor-documented-build-dependencies` 的 `reviewTriggers` 为条件式 soft，命中不阻塞（退出码 0）但 Agent 必须向用户报告；AS 插件 Edit 后自动检查；CI 门禁 `scripts/check-gate.sh`；
