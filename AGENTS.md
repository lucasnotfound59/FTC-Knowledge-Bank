...
你（Agent）不要自己读规则文本做裁决 —— 写码前调用 `ftckb resolve --json` 拿确定性规则，
提交前调用 `ftckb check` 对自己的 diff 执法，让所有 Agent 写出同一标准的代码。
...
- 文档中的规则数/测试数快照（当前 43 条规则：25 条已批准 + 18 条候选、417 项测试）随改动同步更新。
