...
给 Agent 此仓库链接并要求接入目标 FTC 项目；Agent 应读取 [`.agents/skills/ftckb-integrate/SKILL.md`](.agents/skills/ftckb-integrate/SKILL.md)。提供明确队号与赛季；接入与日常规则检查不需要额外 API key。安装、版本固定和恢复见[项目接入指南](docs/project-integration.md)。
...
下一版本候选：新增项目级接入 Skill、固定版本 submodule 安装器及完整 check 契约；尚未发布 tag。`V0.1.1` 文案、CLI `1.0.0` 和 kernel schema `1` 是不同版本层。
...
| 部分完成 | `knowledge/` 当前包含 43 条规则：37 条已批准（1 条官方、30 条共享、6 条 20827 队伍规则，其中 6 条带硬检查）和 6 条候选（含 4 条 Control Hub LED 官方候选）；候选规则保持 inactive，已有 8 篇 SDK、工具、故障排查及项目实践教程，队伍知识内容仍需扩充 |
| 尚未实现 | 审批 UI/历史、Run 模式、官方文档联网检索、Control Hub 部署 |

2026-09-08 核心与 CLI 验证共 422 项 Kotlin 测试：421 项通过、1 项旧安装产物路径假设检查跳过，无失败（JDK 21）；接入 Python 测试 22 项通过，另 1 项真实固定源码构建/CLI 端到端测试单独运行通过。未将 Android Studio 插件全量测试、Windows 原生运行或真机测试计为通过。详见[接入验收记录](docs/project-integration.md#开发验收)。
...
2026-08-20 的历史快照为 417 项测试全部通过；最新验证范围和计数见本文顶部，不能把历史数当作本次全量验证。
