...
采用 RookieBot 新手约定时，业务变量使用角度，并在 Hardwares 中统一换算；见 [0~360 度显示与实际机械行程的区别](../practices/rookiebot-tutorial.md)。本页 0~1 和直接硬件访问片段用于解释底层 SDK/标定，整合进该项目时应移到对应 Hardwares 分组，不能把 360 度软件刻度当作以下所有型号的真实行程。
